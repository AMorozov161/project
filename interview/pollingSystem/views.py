...
from .models import PollingSys, Question, Choice, Answer
from .modelSerializers import PollSerializer, QuestionSerializer, ChoiceSerializer, AnswerSerializer
...
def poll_update(request, poll_id):
    poll = get_object_or_404(PollingSys, pk=poll_id)
...
def active_polls_view(request):
    polls = PollingSys.objects.filter(end_date__gte=timezone.now()).filter(pub_date__lte=timezone.now())
    serializer = PollSerializer(polls, many=True)
    return Response(serializer.data)
